...
test("clients page renders the pruned ADD lead table", async () => {
  ...
  assert.equal(withEmail.length, 36);
  ...
  assert.equal(visitStops.length, 38);
  assert.equal((html.match(/data-visit="true"/g) ?? []).length, 38);
  assert.match(html, /Visit · 38/);
