...
- **Did:** Added Eldreth Law Firm, PLLC (412 Saint Marys St, Ste C, Raleigh NC 27605) to `crabtree-midtown` after Denise Smith Cline. `emailed` false, `outcome` empty, buyer 3, visit true, `ai_visible` false, Instagram blank. Inserted into `today.stopNames` after Bolen and rebuilt Today plus the crabtree Maps loop. Did not remove existing stops or change healthcare entries / emailed flags.
- **Verified:** `npm run build` (15 routes) and `tests/clients-visit-list.test.mjs` passed (40 rows, Visit · 38, Today 19). Local preview at 1440 and 390: Eldreth on General / Law / Visit / Today, Sent unchecked; Healthcare still shows Capital Dermatology and hides Eldreth.
- **Shipped:** PR #61. Merge and make live.
...
- **Did:** Added Bolen Law, PLLC (1508 Brooks Ave, street-level walk-in) and Ballard Law, PLLC (1 Glenwood Ave Ste. 4-132, WeWork, visit false) to `crabtree-midtown` after Paschal. Both `emailed` false, `outcome` empty. Inserted Bolen into `today.stopNames` and rebuilt Today plus the crabtree Maps loop. Did not remove existing stops or change healthcare entries / emailed flags.
- **Verified:** `npm run build` (15 routes) and `tests/clients-visit-list.test.mjs` passed (37 rows, Visit · 36, Today 17). Local preview at 1440 and 390: both names on General / Law, Sent unchecked; Bolen on Visit / Today; Ballard hidden on Visit / Today; Healthcare still shows Capital Dermatology.
- **Shipped:** PR #59. Merge and make live.
...
- **Verified:** `npm run build` (15 routes) and `tests/clients-visit-list.test.mjs` passed (74 rows, Visit · 31). Browser pass at 1440px and 390px on local preview confirmed all five Visit rows with Sent unchecked. Production `https://www.proudlysites.com/clients` returns 200 with all five names and Visit · 31.
- **Shipped:** PR #32 merged to `main` (`4326642`). Production live.
