...
Chosen direction: the services-copy.md "picture-forward proof" pattern rebuilt on the live tokens. Each detail page = hero (eyebrow + outcome H1 + one-line lead), 2-3 real client proof rows (alternating image + decision caption, never a feature list), a two-card "Bigger Picture" bridge to the other two services (the one-person differentiator, stated once per page), and the homepage closing-CTA reused verbatim. The index (redesigned 2026-08-07, branch `redesign/services-editorial`) is now a thesis-led editorial index: a dominant Playfair thesis over three hairline-ruled service rows (name + one-line outcome + terracotta arrow, the whole row linked), with no cards, glyphs, eyebrow, or numbering, then the same close. The three detail pages still use the picture-forward proof template above and are queued for the same editorial treatment.
