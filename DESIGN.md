...
**Static Price Inversion.** The price range callout beside What Moves the Number is the only dark surface in the opening pricing sequence: a static Espresso block on the surrounding Cream field. It carries the public `$4,000–$8,000` band first, then the `$10,000+` complex-project threshold below one quiet hairline. It is never sticky and uses no accent color, gradient, shadow, or motion. At mobile widths it stacks before the five scope-factor rows. Care/Growth and FAQ remain light between this callout and the existing dark closing inquiry, preserving the inversion's contrast authority.
